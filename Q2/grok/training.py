...
class TrainableTransformer(LightningModule):
    ...
    def on_train_epoch_end(self) -> None:
        '''
        """
        Called at the end of the training epoch (v2.0+).
        We replicate the old logic of accumulating losses/accuracies.
        """
        epoch_is_to_be_logged = self.current_epoch == self.next_train_epoch_to_log
        if not epoch_is_to_be_logged:
            return
    ...
    def validation_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            loss, acc, coeff, x_lhs, y_hat_rhs, attentions, values = self._step(
                batch=batch, batch_idx=batch_idx, train=False, reduction="mean"
            )

        # —— 关键：把 val loss/acc log 出去 ——
...
def train(hparams: Namespace) -> None:
    ...
    if hparams.resume_from_checkpoint is not None:
        # 如果命令行里给了 --resume_from_checkpoint，就用它接着训
...
def add_args(parser=None) -> Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="(optional) Path to a .ckpt file to resume training from"
    )
